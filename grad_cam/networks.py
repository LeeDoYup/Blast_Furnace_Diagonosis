...
class VGG16:
    ...
    def create_network(self, input_tensor, label_tensor, is_training, reuse=None):
        ...
        self.output_logit = flatten_convolution(output)
        
        self.output = tf.nn.softmax(self.output_logit)

        print(self.output)

        # losses
        self.loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.label_tensor, logits=self.output_logit)
        print(self.loss)
