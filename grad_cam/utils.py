...
def save_result(image, conv_output, conv_grad, gb_viz, idx=0, version=0, save=True):
    ...
    image_cam = (img + cam_heatmap/255.)/2
    #image_cam255 = (img*255. + cam_heatmap)/2.

    #print(cam_heatmap): 255
    #print(gb_viz): 0-1
    if save:
        if not os.path.exists('./result_img'): os.mkdir('./result_img')
        if version == 0:
            plt.imsave('./result_img/'+str(idx)+'_'+str(version)+'_image.png', img)
        plt.imsave('./result_img/'+str(idx)+'_'+str(version)+'_image_cam.png', image_cam)
        #plt.imsave('./result_img/'+str(idx)+'_'+str(version)+'_image_cam_255.png', image_cam255)
        
        plt.imsave('./result_img/'+str(idx)+'_'+str(version)+'_grad_cam.png', cam_heatmap)
        plt.imsave('./result_img/'+str(idx)+'_'+str(version)+'_guided_bp.png', gb_viz)
        plt.imsave('./result_img/'+str(idx)+'_'+str(version)+'_guided_grad_cam.png', gd_gb)
